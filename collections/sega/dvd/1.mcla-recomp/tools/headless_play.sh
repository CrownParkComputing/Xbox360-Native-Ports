...
FPS_CAP="$(awk -F= '/^[[:space:]]*vsync_fps_cap[[:space:]]*=/{gsub(/[[:space:]]/, "", $2); print $2; exit}' "$ROOT/out/build/linux/midnightclubla.toml")"
VSync="$(awk -F= '/^[[:space:]]*vsync[[:space:]]*=/{gsub(/[[:space:]]/, "", $2); print $2; exit}' "$ROOT/out/build/linux/midnightclubla.toml")"
: "${FPS_CAP:=60}"
: "${VSync:=true}"
HEADLESS_REFRESH="${REX_HEADLESS_REFRESH:-$FPS_CAP}"
[ "$HEADLESS_REFRESH" = "0" ] && HEADLESS_REFRESH=1000
...
gamescope --backend headless -W "${REX_HEADLESS_W:-1280}" -H "${REX_HEADLESS_H:-720}" -r "$HEADLESS_REFRESH" -- env LD_LIBRARY_PATH=. SDL_VIDEODRIVER=x11 REX_VSYNC="$VSync" REX_VSYNC_FPS_CAP="$FPS_CAP" REX_FPS_LOG=1 \
  ${GAME_ENV:-} ./midnightclubla --vsync="$VSync" --vsync_fps_cap="$FPS_CAP" \
  --game_data_root="$ROOT/assets" --gpu_plugin xenos \
  --user_data_root="$FAMILY_DATA" --license_mask=1 --mnk_mode --audio_app_name=rexglue-headless ${GAME_ARGS:-} \
  >"$OUT/run.log" 2>&1 &
GS=$!
...
while [ $((SECONDS - START)) -lt "$SECS" ]; do
  ...
  if [ -n "${PROFILE_AT:-}" ] && [ "$T" -ge "$PROFILE_AT" ] && [ -z "${DONE[profile]:-}" ]; then
    DONE[profile]=1
    PID=$(pgrep -f '(^|/|\s)midnightclubla([[:space:]]|$)' | head -1)
    if [ -z "$PID" ]; then
      echo "could not find midnightclubla process for CPU profile" >&2
      exit 1
    fi
...
pkill -x midnightclubla 2>/dev/null
sleep 3
kill $GS 2>/dev/null
wait 2>/dev/null
