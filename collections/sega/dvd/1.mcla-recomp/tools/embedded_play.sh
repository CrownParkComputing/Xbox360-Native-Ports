...
# rayview watching the dump directory, the launcher (rexmenu) tails the
# shared-memory ring and paints each new frame onto ITS OWN window - raylib is
# the only thing that ever touches the real screen surface, so the guest's
# Vulkan swapchain lives entirely inside the headless compositor and never
# fights the menu for the display. The launcher drives the game through the
# uinput pad it already owns, exactly as rayplay does.
#
# Transports (same contract as the Banjo ports):
...
# - REX_DUMP_SHM (fallback): the runtime publishes each presented frame into a
#   POSIX shared-memory ring - one memcpy, no encoding, no disk, no polling of
#   a directory. The launcher maps the ring and uploads the newest buffer
...
#
# stdout contract (the launcher parses these lines):
#   REXMENU_FRAMEDIR=<path>   PPM fallback directory (always created)
#   REXMENU_SHM=<name>        the shared-memory ring the launcher should map
#   REXMENU_STREAM=<path>     GPU-direct socket (when REX_PRESENT_STREAM is on)
#
...
cp "$ROOT/config/midnightclubla.toml" "$ROOT/out/build/linux/" 2>/dev/null || true
...
SHM_NAME="/rexmenu-frames-mcla"
rm -f "/dev/shm$SHM_NAME"   # never read a stale ring from a previous run
STREAM_SOCK="$OUT/gpu-stream.sock"
rm -f "$STREAM_SOCK"
CMD_ENV=(env)
if [ "${REXMENU_SHM_FALLBACK:-1}" != "0" ]; then
  CMD_ENV+=(REX_DUMP_SHM="$SHM_NAME")
fi
DIAGNOSTIC_ARGS=()
# The timing hooks keep simulation tied to elapsed time. In this mode the
# launcher paces display; the hidden compositor must not add another FIFO wait.
if [ "${MCLA_UNPACED:-1}" = "1" ] && [ "${MCLA_60FPS:-1}" != "0" ]; then
  DIAGNOSTIC_ARGS+=(--vsync=false --vulkan_allow_present_mode_immediate=true
                   --vulkan_allow_present_mode_mailbox=true)
fi
if [ -n "${REX_GPU_FRAME_STATS:-}" ]; then
  DIAGNOSTIC_ARGS+=("--gpu_frame_stats_path=$REX_GPU_FRAME_STATS" --gpu_pass_timing=true)
fi
...
gamescope --backend headless -W 1280 -H 720 -- env LD_LIBRARY_PATH=. SDL_VIDEODRIVER=x11 \
  ./midnightclubla --game_data_root="$ROOT/assets" --gpu_plugin xenos \
  --user_data_root="$FAMILY_DATA" --license_mask=1 --mnk_mode --audio_app_name=rexglue-headless $CAPARG "${DIAGNOSTIC_ARGS[@]}" \
  >"$OUT/run.log" 2>&1 &
GS=$!
...
# the guest dies or the (optional) timer runs out. MCLA spends a while at
# first light (Bink attract + city load), so this waits generously.
