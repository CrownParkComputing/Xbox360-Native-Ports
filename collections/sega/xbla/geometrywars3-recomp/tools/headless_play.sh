...
cp "$ROOT/config/geometrywars3.toml" "$ROOT/out/build/linux/" 2>/dev/null || true
...
gamescope --backend headless -W 1280 -H 720 -- env LD_LIBRARY_PATH=. SDL_VIDEODRIVER=x11 \
  ${GAME_ENV:-} ./geometrywars3 --game_data_root="$ROOT/assets" --gpu_plugin xenos \
...
while [ $((SECONDS - START)) -lt "$SECS" ]; do
  ...
  if [ -n "${PROFILE_AT:-}" ] && [ "$T" -ge "$PROFILE_AT" ] && [ -z "${DONE[profile]:-}" ]; then
    DONE[profile]=1
    PID=$(pgrep -x geometrywars3 | head -1)
...
pkill -x geometrywars3 2>/dev/null
sleep 3
kill $GS 2>/dev/null
wait 2>/dev/null
