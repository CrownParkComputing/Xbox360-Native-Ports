...
# A loose default.xex is useful for bring-up and loader testing even when the
# rest of the content tree is mounted separately.
if [ -f "$SRC" ] && [ "${SRC##*/}" = "default.xex" ]; then
  mkdir -p "$ROOT/assets"
  cp -f "$SRC" "$ROOT/assets/default.xex" ||
    fail "Could not copy default.xex into the Hydro content directory."
  rm -f "$ROOT/assets/.recomp-content-verified"
  say "Loaded default.xex. Import the complete content folder for gameplay."
  exit 0
fi
