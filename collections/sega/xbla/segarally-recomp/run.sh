...
GAME="${SEGARALLY_GAME_DATA:-$ROOT/assets}"
...
if ! "$ROOT/tools/content_zip.sh" verify >/dev/null 2>&1; then
  ZIP="${SEGARALLY_CONTENT_ZIP:-}"
  DEFAULT="$ROOT/content/segarally-content.zip"
  if [ -z "$ZIP" ] && [ -f "$DEFAULT" ]; then
    ZIP="$DEFAULT"
  fi
  if [ -z "$ZIP" ]; then
    if [ -t 0 ]; then
      echo "Game content for segarally is not installed."
      read -r -p "Path to the content zip: " ZIP
    else
      echo "game content missing; set SEGARALLY_CONTENT_ZIP to the archive" >&2
      exit 1
    fi
  fi
  ZIP="${ZIP/#\~/$HOME}"
...
[ -x "$BUILD/segarally" ] || { echo "build first: cmake --build out/build/linux" >&2; exit 1; }
...
cp "$ROOT/config/segarally.toml" "$BUILD/" 2>/dev/null || true

cd "$BUILD"
exec env LD_LIBRARY_PATH=. ./segarally --game_data_root="$GAME" --gpu_plugin xenos \
