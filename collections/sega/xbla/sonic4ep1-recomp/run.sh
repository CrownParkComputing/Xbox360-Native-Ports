...
GAME="${SONIC4EP1_GAME_DATA:-$ROOT/assets}"
...
if ! "$ROOT/tools/content_zip.sh" verify >/dev/null 2>&1; then
  ZIP="${SONIC4EP1_CONTENT_ZIP:-}"
  DEFAULT="$ROOT/content/sonic4ep1-content.zip"
  if [ -z "$ZIP" ] && [ -f "$DEFAULT" ]; then
    ZIP="$DEFAULT"
  fi
  if [ -z "$ZIP" ]; then
    if [ -t 0 ]; then
      echo "Game content for sonic4ep1 is not installed."
      read -r -p "Path to the content zip: " ZIP
    else
      echo "game content missing; set SONIC4EP1_CONTENT_ZIP to the archive" >&2
      exit 1
    fi
  fi
  ZIP="${ZIP/#\~/$HOME}"
...
[ -x "$BUILD/sonic4ep1" ] || { echo "build first: cmake --build out/build/linux" >&2; exit 1; }
...
cp "$ROOT/config/sonic4ep1.toml" "$BUILD/" 2>/dev/null || true

cd "$BUILD"
exec env LD_LIBRARY_PATH=. ./sonic4ep1 --game_data_root="$GAME" --gpu_plugin xenos \
