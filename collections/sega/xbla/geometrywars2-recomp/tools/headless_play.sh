...
cp "$ROOT/config/geometrywars2.toml" "$ROOT/out/build/linux/" 2>/dev/null || true
...
gamescope --backend headless -W 1280 -H 720 -- env LD_LIBRARY_PATH=. SDL_VIDEODRIVER=x11 \
  ${GAME_ENV:-} ./geometrywars2 --game_data_root="$ROOT/assets" --gpu_plugin "$GPU_PLUGIN" \
...
while [ $((SECONDS - START)) -lt "$SECS" ]; do
  ...
  if [ -n "${PROFILE_AT:-}" ] && [ "$T" -ge "$PROFILE_AT" ] && [ -z "${DONE[profile]:-}" ]; then
    DONE[profile]=1
    PID=$(pgrep -x geometrywars2 | head -1)
...
pkill -x geometrywars2 2>/dev/null
sleep 3
kill $GS 2>/dev/null
wait 2>/dev/null
