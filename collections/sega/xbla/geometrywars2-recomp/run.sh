...
GAME="${GEOMETRYWARS2_GAME_DATA:-$ROOT/assets}"
...
if ! "$ROOT/tools/content_zip.sh" verify >/dev/null 2>&1; then
  ZIP="${GEOMETRYWARS2_CONTENT_ZIP:-}"
  DEFAULT="$ROOT/content/geometrywars2-content.zip"
  if [ -z "$ZIP" ] && [ -f "$DEFAULT" ]; then
    ZIP="$DEFAULT"
  fi
  if [ -z "$ZIP" ]; then
    if [ -t 0 ]; then
      echo "Game content for geometrywars2 is not installed."
      read -r -p "Path to the content zip: " ZIP
    else
      echo "game content missing; set GEOMETRYWARS2_CONTENT_ZIP to the archive" >&2
      exit 1
    fi
  fi
  ZIP="${ZIP/#\~/$HOME}"
...
[ -x "$BUILD/geometrywars2" ] || { echo "build first: cmake --build out/build/linux" >&2; exit 1; }
...
for lib in librexruntime.so librexruntimed.so librexruntimerd.so librexgpu-native.so librexgpu-natived.so librexgpu-nativerd.so; do
  ([ ! -f "$BUILD/$lib" ] || [ "$SDK_LIB/$lib" -nt "$BUILD/$lib" ]) && cp "$SDK_LIB/$lib" "$BUILD/" || true
done
cp "$ROOT/config/geometrywars2.toml" "$BUILD/" 2>/dev/null || true

cd "$BUILD"
exec env LD_LIBRARY_PATH=. ./geometrywars2 --game_data_root="$GAME" --gpu_plugin native \
