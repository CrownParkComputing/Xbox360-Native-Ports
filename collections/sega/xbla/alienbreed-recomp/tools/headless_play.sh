...
cp "$ROOT/config/alienbreed.toml" "$ROOT/out/build/linux/" 2>/dev/null || true
...
gamescope --backend headless -W 1280 -H 720 -- env LD_LIBRARY_PATH=. SDL_VIDEODRIVER=x11 \
  ${GAME_ENV:-} ./alienbreed --game_data_root="$ROOT/assets" --gpu_plugin xenos \
