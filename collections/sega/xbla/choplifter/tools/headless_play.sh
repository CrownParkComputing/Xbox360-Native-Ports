...
cp "$ROOT/config/choplifter.toml" "$ROOT/out/build/linux/" 2>/dev/null || true
...
for lib in librexruntime.so librexgpu-xenos.so; do
  [ "/home/jon/rexglue-vmx/out/install/linux-amd64/lib/$lib" -nt "$ROOT/out/build/linux/$lib" ] \
    && cp "/home/jon/rexglue-vmx/out/install/linux-amd64/lib/$lib" "$ROOT/out/build/linux/" || true
done
...
gamescope --backend headless -W "${REX_HEADLESS_W:-1280}" -H "${REX_HEADLESS_H:-720}" -r "${REX_HEADLESS_REFRESH:-60}" -- env LD_LIBRARY_PATH=. SDL_VIDEODRIVER=x11 \
  ${GAME_ENV:-} ./choplifter --game_data_root="$ROOT/assets" --gpu_plugin xenos \
