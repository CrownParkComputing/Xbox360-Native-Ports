...
cp "$ROOT/config/geometrywars.toml" "$ROOT/out/build/linux/" 2>/dev/null || true
...
gamescope --backend headless -W "${REX_HEADLESS_W:-1280}" -H "${REX_HEADLESS_H:-720}" -r "${REX_HEADLESS_REFRESH:-60}" -- env LD_LIBRARY_PATH=. SDL_VIDEODRIVER=x11 \
  ${GAME_ENV:-} ./geometrywars --game_data_root="$ROOT/assets" --gpu_plugin xenos \
...
while [ $((SECONDS - START)) -lt "$SECS" ]; do
  ...
  if [ -n "${PROFILE_AT:-}" ] && [ "$T" -ge "$PROFILE_AT" ] && [ -z "${DONE[profile]:-}" ]; then
    DONE[profile]=1
    PID=$(pgrep -x geometrywars | head -1)
...
pkill -x geometrywars 2>/dev/null
sleep 3
kill $GS 2>/dev/null
wait 2>/dev/null
