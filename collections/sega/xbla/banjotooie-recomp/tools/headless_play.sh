...
cp "$ROOT/config/banjotooie.toml" "$ROOT/out/build/linux/" 2>/dev/null || true
...
# Same family user-data root run.sh uses (see run.sh): saves and profile are
# shared across the ports, and a headless run must measure the same layout a
# player gets.
FAMILY_DATA="${BANJOTOOIE_USER_DATA:-$ROOT/../_shared/user-data}"
[ -d "$FAMILY_DATA" ] || FAMILY_DATA="$ROOT/user-data"

gamescope --backend headless -W "${REX_HEADLESS_W:-1280}" -H "${REX_HEADLESS_H:-720}" -r "${REX_HEADLESS_REFRESH:-60}" -- env LD_LIBRARY_PATH=. SDL_VIDEODRIVER=x11 \
  ${GAME_ENV:-} ./banjotooie --game_data_root="$ROOT/assets" --gpu_plugin xenos \
