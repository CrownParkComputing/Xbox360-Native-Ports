...
GAME="${BANJOKAZOOIE_GAME_DATA:-$ROOT/assets}"
...
if ! "$ROOT/tools/content_zip.sh" verify >/dev/null 2>&1 && [ ! -f "$GAME/default.xex" ]; then
  ZIP="${BANJOKAZOOIE_CONTENT_ZIP:-}"
  DEFAULT="$ROOT/content/banjokazooie-content.zip"
  if [ -z "$ZIP" ] && [ -f "$DEFAULT" ]; then
    ZIP="$DEFAULT"
  fi
  if [ -z "$ZIP" ]; then
    if [ -t 0 ]; then
      echo "Game content for banjokazooie is not installed."
      read -r -p "Path to the content zip: " ZIP
    else
      echo "game content missing; set BANJOKAZOOIE_CONTENT_ZIP to the archive" >&2
      exit 1
    fi
  fi
  ZIP="${ZIP/#\~/$HOME}"
...
[ -x "$BUILD/banjokazooie" ] || { echo "build first: cmake --build out/build/linux" >&2; exit 1; }
...
cp "$ROOT/config/banjokazooie.toml" "$BUILD/" 2>/dev/null || true
...
# the other titles' saves exactly as the console did (Tooie reads the Kazooie
# save for Stop 'N' Swop). Shader caches under it are keyed by title id, so the
# titles do not fight over one cache.
FAMILY_DATA="${BANJOKAZOOIE_USER_DATA:-$ROOT/../_shared/user-data}"
if [ ! -d "$FAMILY_DATA" ]; then FAMILY_DATA="$ROOT/user-data"; fi

cd "$BUILD"
exec env LD_LIBRARY_PATH=. ./banjokazooie --game_data_root="$GAME" --gpu_plugin xenos \
