...
have_gui() { [ -z "${REX_MENU_QUIET:-}" ] && [ -n "${DISPLAY:-}${WAYLAND_DISPLAY:-}" ] && command -v zenity >/dev/null 2>&1; }
...
# The root is the FAMILY user-data shared with the other ports in this tree,
# so DLC lands where the games' shared profile can see it.
DLC_ROOT="${BANJOKAZOOIE_USER_DATA:-$ROOT/../_shared/user-data}"
[ -d "$DLC_ROOT" ] || DLC_ROOT="$ROOT/user-data"
DLC_ROOT="$DLC_ROOT/0000000000000000"
...
TREE="$SRC"
DIRECT_XEX=""
if [ -f "$SRC" ] && [[ "${SRC,,}" == *.xex ]]; then
  DIRECT_XEX="$SRC"
  echo "Selected XEX: $(basename "$SRC")"
  echo "Importing the executable into the game content folder..."
fi
if [ -f "$SRC" ]; then
  if [ -n "$DIRECT_XEX" ]; then
    TREE="$(dirname "$DIRECT_XEX")"
  else
...
XEX="$DIRECT_XEX"
[ -z "$XEX" ] && XEX="$(find "$TREE" -iname 'default.xex' -type f 2>/dev/null | head -1)"
if [ -z "$XEX" ]; then
...
echo "Progress: copying XEX and game files..."
mkdir -p "$ROOT/assets"
...
if [ -n "$DIRECT_XEX" ]; then
  cp -f "$DIRECT_XEX" "$ROOT/assets/default.xex"
else
