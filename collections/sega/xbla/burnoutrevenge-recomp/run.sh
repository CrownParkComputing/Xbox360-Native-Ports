...
GAME="${BURNOUTREVENGE_GAME_DATA:-$ROOT/assets}"
...
if ! "$ROOT/tools/content_zip.sh" verify >/dev/null 2>&1; then
  ZIP="${BURNOUTREVENGE_CONTENT_ZIP:-}"
  DEFAULT="$ROOT/content/burnoutrevenge-content.zip"
  if [ -z "$ZIP" ] && [ -f "$DEFAULT" ]; then
    ZIP="$DEFAULT"
  fi
  if [ -z "$ZIP" ]; then
    if [ -t 0 ]; then
      echo "Game content for burnoutrevenge is not installed."
      read -r -p "Path to the content zip: " ZIP
    else
      echo "game content missing; set BURNOUTREVENGE_CONTENT_ZIP to the archive" >&2
      exit 1
    fi
  fi
  ZIP="${ZIP/#\~/$HOME}"
...
[ -x "$BUILD/burnoutrevenge" ] || { echo "build first: cmake --build out/build/linux" >&2; exit 1; }
...
cp "$ROOT/config/burnoutrevenge.toml" "$BUILD/" 2>/dev/null || true

cd "$BUILD"
exec env LD_LIBRARY_PATH=. ./burnoutrevenge --game_data_root="$GAME" --gpu_plugin xenos \
